...
export default defineConfig({
  ...
  manifest: ({ browser }) => ({
    ...
    permissions: ['storage', 'activeTab', 'alarms', ...(browser === 'firefox' ? [] : ['sidePanel'])],
    host_permissions: [...ATS_MATCH_PATTERNS],
    // Requested from the options page only when the user enables AI assistance
    // and supplies their own key (§6.4) — never granted by default.

...
import type { FieldMapping, FillPlan } from '@/shared/types';
...
export async function planFill(request: PlanRequest): Promise<FillPlan> {
  ...
  const plan = buildFillPlan(request.fields, mappings, profile, { fillEeo: settings.fillEeo });
  log.info(
    `planned ${plan.instructions.length} fills, ${plan.skipped.length} skipped on ${request.hostname}`,
  );
  return plan;
}

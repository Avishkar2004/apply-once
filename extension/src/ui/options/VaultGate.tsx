  ...
  const submit = async () => {
    ...
    try {
      const next = useRecovery
        ? await sendToBackground("session:unlock-recovery", {
            code: recoveryCode,
          })
  ...
  return (
    <Card className="mx-auto max-w-md">
      ...
      <p className="mt-1 mb-4 text-sm text-slate-500 dark:text-slate-400">
        Your profile is encrypted on this device. It stays unlocked until you
        close the browser.
      </p>
...
function CreateVaultForm({
  ...
  const create = async () => {
    if (passphrase.length < 10) {
      setError(
        "Use at least 10 characters. This passphrase is the only thing protecting your profile.",
      );
      return;
    }
    ...
    setError(undefined);
    try {
      const { recoveryCode } = await sendToBackground("session:create", {
        passphrase,
      });
      setKit(recoveryCode);
    } catch (cause) {
    ...
    return (
      <Card className="mx-auto max-w-md">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          This is the only way back in if you forget your passphrase. Nobody can
          reset it for you.
        </p>
        ...
        <Banner tone="warn">
          Print this or write it down. Do not store it in the same password
          manager as your passphrase.
        </Banner>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <Button
            variant="secondary"
            onClick={() => void navigator.clipboard.writeText(kit.formatted)}
          >
            Copy
          </Button>
          <Button
  ...
  return (
    <Card className="mx-auto max-w-md">
      ...
      <p className="mt-1 mb-4 text-sm text-slate-500 dark:text-slate-400">
        Choose a passphrase. Your profile is encrypted with it on this device
        and never leaves unless you turn on sync.
      </p>

...
import { ANTHROPIC_ORIGIN, requestApiHostPermission, revokeApiHostPermission } from '@/llm/credentials';
...
export function AiPanel() {
  ...
  const saveKey = async () => {
    ...
    try {
      ...
      setApiKeyInput('');
      setMessage({ tone: 'success', text: 'API key saved. AI assistance is on.' });
      await refresh();
    } catch (cause) {
  ...
  const removeKey = async () => {
    ...
    try {
      ...
      setMessage({ tone: 'success', text: 'API key removed and permission revoked.' });
      await refresh();
    } finally {
      setBusy(false);
    }
  ...
  const embeddingsLabel = {
    ...
    unavailable: 'Not installed — run `npm run fetch:model`. Mapping still works without it.',
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="sm:col-span-2">
          <Banner tone="info">
            ...
            <span className="mt-1 block">
              Mapping an unfamiliar field sends its <em>label and options only</em> — never your
              profile data. Drafting an answer to a written question does send your profile and
              résumé text, because the draft has to be about you. Requests go directly from this
              browser to Anthropic using your own API key.
            </span>
          </Banner>
        </div>
        ...
        {status?.hasKey ? (
          <>
            <div className="sm:col-span-2 space-y-1 text-sm text-slate-600 dark:text-slate-300">
              <p>
                ✅ API key stored, encrypted with your passphrase.
                {status.hasHostPermission ? ' Host permission granted.' : ' Host permission missing.'}
              </p>
            ...
            <CheckboxField
              ...
              onChange={(llmEnabled) => {
                void sendToBackground('settings:set', { llmEnabled }).then(refresh);
              }}
            />
            <div className="sm:col-span-2">
          ...
          <>
            ...
            />
            <div className="sm:col-span-2">
              <Button variant="primary" disabled={busy || apiKey.length < 20} onClick={() => void saveKey()}>
                {busy ? 'Saving…' : 'Save key and grant access'}
              </Button>
              <p className="mt-2 text-xs text-slate-500">
                Clicking this asks Chrome for permission to reach {ANTHROPIC_ORIGIN}. Nothing is sent
                until you fill a form that needs it.
              </p>
            </div>
          </>
        )}

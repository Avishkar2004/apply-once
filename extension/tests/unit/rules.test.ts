...
function field(
  labels: string[],
  extra: Partial<FieldDescriptorDto> = {},
): FieldDescriptorDto {
  return {
    id: "f",
    signature: "s",
...
describe("Tier 1 label rules", () => {
  ...
  it("does not put a company name into the applicant surname (§3.2)", () => {
    expect(mapWithRules(field(["Company Name"]))?.key).not.toBe(
      "personal.lastName",
    );
    expect(mapWithRules(field(["Company Name"]))?.key).not.toBe(
      "personal.fullName",
  ...
  it("does not treat a file name or user name as the applicant name", () => {
    expect(mapWithRules(field(["File name"]))?.key).not.toBe(
      "personal.fullName",
    );
    expect(mapWithRules(field(["User name"]))?.key).not.toBe(
      "personal.fullName",
    );
  });

  it("keeps a confirmation email out of the contact email", () => {
    expect(mapWithRules(field(["Confirm email"]))?.key).not.toBe(
      "contact.email",
    );
  });

  it("keeps the phone country code out of the phone number", () => {
    expect(mapWithRules(field(["Phone country code"]))?.key).toBe(
  ...
  it("leaves genuinely novel questions unmapped for a later tier", () => {
    expect(
      mapWithRules(
        field(["Which of our engineering values resonates most with you?"]),
      ),
    ).toBeNull();
  });
  ...
  it("restricts file-only rules to file controls", () => {
    expect(mapWithRules(field(["Resume"], { kind: "file" }))?.key).toBe(
      "documents.resume",
    );
    expect(mapWithRules(field(["Resume"], { kind: "text" }))?.key).not.toBe(
      "documents.resume",
    );
  });
});
...
describe("section heading disambiguation (§11)", () => {
  it("routes a start date under an Education heading to education", () => {
    const hit = mapWithRules(
      field(["Start date"], { sectionHeading: "Education" }),
    );
  ...
  it("routes contact details under a References heading to the reference", () => {
    expect(
      mapWithRules(field(["Email"], { sectionHeading: "Reference 1" }))?.key,
    ).toBe("references[].email");
  });
});

describe("autocomplete rules", () => {
  it("strips section and address-type prefixes", () => {
    expect(normalizeAutocomplete("section-blue shipping given-name")).toBe(
      "given-name",
    );
  });
  ...
  ] as Array<[string, CanonicalKey]>)(
    "maps autocomplete=%s → %s",
    (token, expected) => {
      expect(
        matchAutocomplete(field(["irrelevant"], { autocomplete: token }))?.key,
      ).toBe(expected);
    },
  );
  ...
  it("beats the label rules when both would fire", () => {
    ...
    const hit = mapWithRules(
      field(["Company"], { autocomplete: "given-name" }),
    );
...
describe("vocabulary outside the US", () => {
  ...
  it("does not mistake the expected figure for the current one", () => {
    expect(
      mapWithRules(field(["Expected CTC"], { kind: "number" }))?.key,
    ).not.toBe("preferences.currentSalary.amount");
  });
  ...
  it("leaves a written question about experience to the answer generator", () => {
    ...
    expect(
      mapWithRules(field(["Describe your experience"], { kind: "textarea" }))
        ?.key,
    ).not.toBe("work.totalYears");
  });

  it("still prefers a named document over the generic attachment rule", () => {
    expect(
      mapWithRules(field(["Attach your cover letter"], { kind: "file" }))?.key,
    ).toBe("documents.coverLetter");
  });
});

/**
 * A cover letter is asked for two ways, and they are not interchangeable.
 *
 * Naukri's "Cover Letter / Message *" is a textarea. Attaching a PDF to it is
 * impossible, and the file rule was restricted to `kinds: ['file']`, so the
 * field mapped to nothing at all and came back ⬜ on every application.
 */
describe("cover letter, as a file and as prose", () => {
  it("sends an upload box to the stored document", () => {
    expect(mapWithRules(field(["Cover letter"], { kind: "file" }))?.key).toBe(
      "documents.coverLetter",
    );
  });

  it("sends a textarea to the written one", () => {
    expect(
      mapWithRules(field(["Cover Letter / Message *"], { kind: "textarea" }))
        ?.key,
    ).toBe("documents.coverLetterText");
  });
  ...
  ])("maps %s in a textarea to the written letter", (label) => {
    expect(mapWithRules(field([label], { kind: "textarea" }))?.key).toBe(
      "documents.coverLetterText",
    );
  });

  it("does not let the written letter claim a file input", () => {
    expect(
      mapWithRules(field(["Cover Letter / Message"], { kind: "file" }))?.key,
    ).toBe("documents.coverLetter");
  });

  it("leaves an unrelated message box alone", () => {
    // A contact form's "Message" is not a cover letter.
    expect(
      mapWithRules(field(["Message"], { kind: "textarea" }))?.key,
    ).not.toBe("documents.coverLetterText");
  });
});
